<x-guest-layout>
    ...
    <div class="min-h-screen w-full">
        <div class="flex items-center justify-center">
            ...
            <div class="lg:col-span-2 w-full p-8 max-w-xl max-md:max-w-xl mx-auto mt-12">

                <div class="mb-4">
                    <x-auth-session-status class="mb-4" :status="session('status')" />
                    <x-session-error-status class="mb-4" :error="session('error')" />
                    <x-session-info-status class="mb-4" :info="session('info')" />
                    ...
                    <form method="POST" action="{{ route('verify.account.process') }}" class="w-full">
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <button type="submit"
                                class="w-full py-2.5 px-4 text-[15px] font-medium tracking-wide rounded-md text-white bg-primary-900 hover:bg-primary-700 focus:outline-none cursor-pointer">
                                Verify Account
                            </button>
                        </div>
